...
# TODO: not compatible with Terraform 0.12. Fix it if these get used.
...
/*data "template_file" "lifecycle_policy_rules" {
  ...
  template = replace(
    var.lifecycle_policy_rules[count.index],
    "priority:replace:this",
    count.index + 1,
  )
}*/
...
/*resource "aws_ecr_lifecycle_policy" "this" {
  ...
  policy = replace(
    "{\"rules\": [${join(",", data.template_file.lifecycle_policy_rules.*.rendered)}]}",
    "/\"(true|false|[[:digit:]]+)\"/",
    "$1",
  )
}*/
